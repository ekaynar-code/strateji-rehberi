...
import { fuarCsvIceAktar, FUAR_CSV_ORNEK_BASLIK, type FuarCsvSatirSonucu } from "@/lib/fuarlar";

export default function FuarCsvIceAktarForm({ onDone }: { onDone: () => void }) {
  const [yukleniyor, setYukleniyor] = useState(false);
  const [sonuclar, setSonuclar] = useState<FuarCsvSatirSonucu[] | null>(null);
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const sonuc = await fuarCsvIceAktar(metin);
      setSonuclar(sonuc);
    } catch {
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-4 sm:p-5">
      ...
      <code className="mb-3 block overflow-x-auto rounded-lg bg-gray-50 px-3 py-2 text-xs text-gray-600">
        {FUAR_CSV_ORNEK_BASLIK}
      </code>
      <p className="mb-4 text-xs text-gray-400">
        bolge: korfez / balkanlar / afrika / yurt_ici / diger · tarih: YYYY-MM-DD formatında ·
        notlar opsiyoneldir.
      </p>
